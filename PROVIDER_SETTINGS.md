...
The FlockPoster CLI supports platform-specific settings for each integration. Different platforms have different options and requirements.
...
```bash
flockposter posts:create --json post-with-settings.json
```
...
```bash
flockposter posts:create --json reddit-post.json
```
...
```bash
flockposter posts:create --json youtube-video.json
```
