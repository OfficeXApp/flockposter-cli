...
```bash
flockposter integrations:settings reddit-abc123
```
...
```bash
flockposter integrations:trigger reddit-abc123 getFlairs -d '{"subreddit":"programming"}'
```
...
```bash
flockposter integrations:settings youtube-123
```
...
```bash
flockposter integrations:trigger youtube-123 getPlaylists
```
...
```bash
flockposter integrations:settings linkedin-123
```
...
```bash
flockposter integrations:trigger linkedin-123 getCompanies
```
...
```bash
# No parameters
flockposter integrations:trigger <integration-id> <methodName>

# With parameters
flockposter integrations:trigger <integration-id> <methodName> -d '{"key":"value"}'
```
...
SETTINGS=$(flockposter integrations:settings "$INTEGRATION_ID")
...
for METHOD in $TOOLS; do
  RESULT=$(flockposter integrations:trigger "$INTEGRATION_ID" "$METHOD" -d '{}')
...
```bash
flockposter integrations:trigger reddit-123 invalidMethod
...
```bash
flockposter integrations:trigger reddit-123 getFlairs
...
```bash
flockposter integrations:trigger invalid-id getFlairs
...
FLAIRS=$(flockposter integrations:trigger $INTEGRATION_ID getFlairs -d '{"subreddit":"programming"}')
