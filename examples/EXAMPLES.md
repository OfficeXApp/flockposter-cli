# FlockPoster CLI - Advanced Examples

This directory contains examples demonstrating the full capabilities of the FlockPoster CLI, including posts with comments and multiple media.

## Understanding the Post Structure

The FlockPoster API supports a rich post structure:
...
```bash
flockposter posts:create --json examples/post-with-comments.json
```
...
```bash
flockposter posts:create --json examples/multi-platform-post.json
```
...
```bash
flockposter posts:create --json examples/thread-post.json
```
...
for file in posts/*.json; do
  ...
  flockposter posts:create --json "$file"
  sleep 2
done
```
...
flockposter posts:create --json thread.json
```
...
2. **Invalid image path** - Ensure images are accessible URLs or uploaded to FlockPoster first
